...
CONTAINS
  ...
  Subroutine derivs(x,y,dydx)
    ...
    i_t=floor(x)
    !write (*,*) i_t
    if ((i_t.ge.1).and.(i_t.le.tot_h))&
           dydx(:,:,1) = dydx(:,:,1)+(0.0,1.0)*pot_c_t(:,:,i_t)*y(:,:,1)
    !exciton part
    dydx(:,:,2)= -kappa_X*y(:,:,2)+&    
  ...
  CONTAINS    
    SUBROUTINE save_a_step_sp    
      ...
      do h_save=1, tot_save

          if( x.ge.t_stfft+kount*dxsav_sp .and.&
              kount.ge.(h_save-1)*Nt .and. kount.le.h_save*Nt-1 ) then
             i_t=(kount+1)-(h_save-1)*Nt
             write(file_time,*) kount, x, i_t
             call flush(file_time)
             y_tot(:,:,i_t)=y(:,:,1)
             kount=kount+1
          end if

          if (kount.eq.h_save*Nt .and. check(h_save).eq.0) then 
             call eval_spectr
             n_save=n_save+1
